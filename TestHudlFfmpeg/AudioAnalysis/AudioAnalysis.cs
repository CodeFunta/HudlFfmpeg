...
namespace FinalYearProject_11010841
{
    class AudioAnalysis
    {
        ...
        public List<TimeSpan> GetNormilizedOnsets(TimeSpan onsetStepDurationMax,float sensitivity = 1.5f, float thresholdTimeSpan = 0.5f)
        {
            List<TimeSpan> retval = new List<TimeSpan>();
            this.DetectOnsets(sensitivity, thresholdTimeSpan);
            // Normalize the intensity of the onsets
            this.NormalizeOnsets(0);
            float[] onsets = this.GetOnsets();
            // Length of time each item in the onset detected list represents
            float timePos = this.GetTimePerSample();

            
            TimeSpan curDurration = TimeSpan.Zero;
            for (int itemNo = 0; itemNo < onsets.Length; itemNo++)
            {
                // Retrieve the item from the audio analysis onset detected list
                float onset = onsets[itemNo];

                // If the item is an onset
                if (onset > 0.0)
                {
                    // Place the note in accordance to its position in the song
                    TimeSpan xPosition = TimeSpan.FromSeconds(itemNo * timePos);
                    TimeSpan lastDurration = retval.LastOrDefault();
                    if (xPosition.Subtract(lastDurration) > onsetStepDurationMax)
                    {
                        retval.Add(xPosition);
                    }
